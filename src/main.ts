...
// ====== Config ======
const accessWords = ["SIGNAL", "ACCESS", "CODEX", "VECTOR", "ALPHA"];
const accessWord = accessWords[Math.floor(Math.random() * accessWords.length)];
let typedChars = "";
...
// ====== Setup ======
pre.innerHTML = chars.map(c =>
  `<span data-char="${c}" data-glitching="false">${c === ' ' ? '&nbsp;' : c}</span>`
).join('');
...
function glitchLoop() {
  ...
  for (const { span, x, y } of rects) {
    ...
    if (!inRange || span.dataset.glitching === "true" || span.dataset.glitchKey === "typed" || originalChar.trim() === '') continue;
...
document.addEventListener("keydown", (e) => {
  ...
  const target = Array.from(spanElems).find(span =>
  ...
  if (typedChars === accessWord) {
    setTimeout(() => window.location.href = "/granted.html", 1000);
  }
});

console.log("Access word:", accessWord);
